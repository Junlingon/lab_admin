  ...
  <div class="userlist">
    ...
    <el-card class="mt20">
      <el-table
        ...
        scrollbar-always-on
      >
        <el-table-column label="实验室名称" prop="labName" />
        ...
        <el-table-column label="简介" prop="labInfo" />
...
const getlist = async (e) => {
  const res = await proxy.$api.AdminApi.laborList({
  ...
  // });
  res.data.list.forEach((item) => {
    item.labImg = item.labImg ? `${proxy.$utils.imgurl}${item.labImg}` : "";
    item.srcList = [item.labImg];
  });
