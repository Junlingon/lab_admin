  ...
  <div class="userlist">
    ...
    <el-card class="mt20">
      ...
      >
        ...
        <!-- <el-table-column label="盐" prop="salt" /> -->
    ...
    <RootAdminUpd @savebtn="updsavebtn" ref="RootAdminUpdref"></RootAdminUpd>
    <RootAdminDetails
      @savebtn="delsavebtn"
      ref="RootAdminDetailsref"
    ></RootAdminDetails>
  </div>
</template>

<script setup>
import RootAdminUpd from "@/view/admin/root/root_admin_upd.vue"; //新增修改组件
import RootAdminDetails from "@/view/admin/root/root_admin_details.vue"; //详情组件
...
const RootAdminUpdref = ref(null); //新增 修改组件
const RootAdminDetailsref = ref(null); //详情组件
//搜索
const searchbtn = () => {
  getlist();
};
...
const frozenbtn = async (e) => {
  const res = await proxy.$api.AdminApi.NoDelAdmin({
    id: e.id,
  });
  await getlist();
};
...
const detailsbtn = (type, e) => {
  RootAdminDetailsref.value.blockfunc(type, e);
};
...
const updbtn = async (type, e) => {
  RootAdminUpdref.value.blockfunc(type, e);
};
...
const getlist = async (e) => {
  const res = await proxy.$api.AdminApi.AdminList({
...
//重置密码
const creatpwd = (e) => {
  ElMessageBox.confirm("请谨慎操作，您确定重置密码吗?", "提示", {
